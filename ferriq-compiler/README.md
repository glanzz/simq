...
Circuit optimization and compilation for the Ferriq quantum computing SDK.

## Overview

The `ferriq-compiler` crate provides optimization passes that transform quantum circuits into more efficient representations. The primary focus is on reducing circuit depth and gate count while preserving quantum behavior.
...
```rust
use ferriq_compiler::fusion::{fuse_single_qubit_gates, FusionConfig};
...
```bash
cargo test --package ferriq-compiler
```
...
```bash
cargo bench --package ferriq-compiler
```
...
```bash
cargo run --package ferriq-compiler --example gate_fusion
```
