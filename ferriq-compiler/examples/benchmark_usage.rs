...
use ferriq_compiler::{create_compiler, OptimizationLevel};
use ferriq_core::{Circuit, QubitId};
use ferriq_gates::standard::{Hadamard, PauliX, PauliY, PauliZ, RotationZ, SGate, TGate};
use std::sync::Arc;
...
fn create_benchmark_circuit(num_qubits: usize) -> Circuit {
    ...
    for qubit_idx in 0..num_qubits {
        ...
        if qubit_idx + 1 < num_qubits {
            ...
            circuit
                .add_gate(Arc::new(ferriq_gates::standard::CNot), &[qubit, next_qubit])
                .unwrap();
        }
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("╔═══════════════════════════════════════════════════════════╗");
    println!("║   Ferriq Compiler Optimization Pass Benchmarks             ║");
    println!("╚═══════════════════════════════════════════════════════════╝\n");
