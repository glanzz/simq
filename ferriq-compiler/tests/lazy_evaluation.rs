...
use ferriq_compiler::lazy::{LazyConfig, LazyExecutor};
...
fn test_lazy_executor_two_qubit_gates() {
    ...
    // CNOT(q0->q1) should flip q1 when q0=1, giving the Bell state (|00⟩ + |11⟩)/√2
    let amps = state.amplitudes();
    let expected_re = 1.0 / 2.0_f64.sqrt();
    ...
    assert_relative_eq!(amps[3].norm(), expected_re, epsilon = EPSILON);
}
...
fn test_lazy_executor_cache_eviction() {
    ...
    for gate in [
        Arc::new(Hadamard) as Arc<dyn ferriq_core::gate::Gate>,
        Arc::new(PauliX) as Arc<dyn ferriq_core::gate::Gate>,
        Arc::new(PauliY) as Arc<dyn ferriq_core::gate::Gate>,
    ] {
        let mut builder = CircuitBuilder::<1>::new();
        let [q0] = builder.qubits();
