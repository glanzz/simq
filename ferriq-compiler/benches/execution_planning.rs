...
use ferriq_gates::standard::{CNot, Hadamard, PauliX, PauliY, PauliZ, SGate, TGate};
use std::sync::Arc;
...
fn create_sequential_circuit(num_gates: usize) -> Circuit {
    ...
    for i in 0..num_gates {
        let gate: Arc<dyn ferriq_core::gate::Gate> = match i % 4 {
            0 => Arc::new(Hadamard),
            1 => Arc::new(PauliX),
            2 => Arc::new(PauliY),
            _ => Arc::new(PauliZ),
...
fn create_mixed_circuit(num_qubits: usize, depth: usize) -> Circuit {
    ...
    for layer in 0..depth {
        ...
        for q in 0..num_qubits {
            let gate: Arc<dyn ferriq_core::gate::Gate> = match layer % 3 {
                0 => Arc::new(Hadamard),
                1 => Arc::new(SGate),
                _ => Arc::new(TGate),
            };
...
fn create_deep_circuit(num_qubits: usize, depth: usize) -> Circuit {
    ...
    for d in 0..depth {
        for q in 0..num_qubits {
            let gate: Arc<dyn ferriq_core::gate::Gate> = match (d + q) % 5 {
                0 => Arc::new(Hadamard),
                1 => Arc::new(PauliX),
                2 => Arc::new(PauliY),
                3 => Arc::new(PauliZ),
