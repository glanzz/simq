...
use ferriq_core::{Circuit, Gate, GateOp, QubitId};
...
impl ExecutionEngine {
    ...
    fn apply_gate_dense(
        ...
        gate: &Arc<dyn Gate>,
        qubits: &[QubitId],
        state: &mut ferriq_state::DenseState,
        use_parallel: bool,
        threshold: usize,
    ) -> Result<()> {
    ...
    fn apply_gate_sparse(
        ...
        gate: &Arc<dyn Gate>,
        qubits: &[QubitId],
        state: &mut ferriq_state::SparseState,
        _use_parallel: bool,
        _threshold: usize,
    ) -> Result<()> {
        let gate_name = gate.name();
...
mod tests {
    ...
    impl ferriq_core::Gate for NoMatrixGate {
        fn name(&self) -> &str {
            "NoMatrixGate"
        }
    ...
    impl ferriq_core::Gate for BadMatrixGate {
        fn name(&self) -> &str {
            "BadMatrixGate"
        }
        fn num_qubits(&self) -> usize {
    ...
    impl ferriq_core::Gate for FourQubitGate {
        fn name(&self) -> &str {
            "FourQubitGate"
        }
        fn num_qubits(&self) -> usize {
    ...
    impl ferriq_core::Gate for BadMatrix2qGate {
        fn name(&self) -> &str {
            "BadMatrix2qGate"
        }
