...
impl From<ferriq_core::QuantumError> for SimulatorError {
    fn from(err: ferriq_core::QuantumError) -> Self {
        SimulatorError::InvalidCircuit(err.to_string())
    }
}

impl From<ferriq_state::error::StateError> for SimulatorError {
    fn from(err: ferriq_state::error::StateError) -> Self {
...
mod tests {
    ...
    fn test_from_quantum_error() {
        let qe = ferriq_core::QuantumError::EmptyCircuit;
    ...
    fn test_from_state_error() {
        let se = ferriq_state::error::StateError::InvalidDimension { dimension: 3 };
