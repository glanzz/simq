...
use ferriq_gates::standard::{CNot, RotationY, RotationZ};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let initial_energy = match &initial_result.state {
        ferriq_state::AdaptiveState::Dense(dense) => observable.expectation_value(dense)?,
        ferriq_state::AdaptiveState::Sparse { state: sparse, .. } => {
...
fn print_trajectory(history: &[ferriq_sim::gradient::OptimizationStep], n: usize) {
