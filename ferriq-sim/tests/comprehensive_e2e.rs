//! Comprehensive end-to-end tests for ferriq-sim crate
//!
...
fn execution_engine_basic() {
    use ferriq_sim::execution_engine::{ExecutionConfig, ExecutionEngine};
    let config = ExecutionConfig::default();
    let mut engine = ExecutionEngine::new(config);
    let circuit = bell_circuit();
    let mut state = ferriq_state::AdaptiveState::new(2).unwrap();
...
fn execution_engine_single_gate() {
    ...
    let mut engine = ExecutionEngine::new(config);
    let circuit = single_x_circuit();
    let mut state = ferriq_state::AdaptiveState::new(1).unwrap();
    let result = engine.execute(&circuit, &mut state);
    assert!(result.is_ok());
}
...
fn qaoa_circuit_basic() {
    ...
    let circuit = ferriq_sim::qaoa_circuit(2, &cost_h, &mixer, 1, &params);
...
fn qaoa_circuit_multi_layer() {
    ...
    let circuit = ferriq_sim::qaoa_circuit(3, &cost_h, &mixer, 2, &params);
...
fn vqe_ansatz_basic() {
    ...
    let circuit = ferriq_sim::vqe_hardware_efficient_ansatz(3, &params);
    assert_eq!(circuit.num_qubits(), 3);
    assert!(!circuit.is_empty());
}
...
fn vqe_ansatz_simulatable() {
    let sim = Simulator::new(SimulatorConfig::default().with_optimization(false));
    let params = vec![0.5, 0.3];
    let circuit = ferriq_sim::vqe_hardware_efficient_ansatz(2, &params);
    let result = sim.run(&circuit);
    assert!(result.is_ok());
}
...
fn gradient_config_default() {
    let config = ferriq_sim::gradient::GradientConfig::default();
    assert_eq!(config.method, ferriq_sim::gradient::GradientMethod::ParameterShift);
    assert!(config.parallel);
}

#[test]
fn gradient_result_norm() {
    let result = ferriq_sim::gradient::GradientResult {
        gradients: vec![3.0, 4.0],
        num_evaluations: 4,
        computation_time: std::time::Duration::from_millis(1),
        method_used: ferriq_sim::gradient::GradientMethod::FiniteDifference,
    };
...
fn convergence_monitor() {
    let mut config = ferriq_sim::gradient::MonitorConfig::default();
    config.patience = 5;
    config.window_size = 3;
    let mut monitor = ferriq_sim::gradient::ConvergenceMonitor::new(config);
    for i in 0..10 {
...
#[test]
fn best_tracker() {
    let mut tracker = ferriq_sim::gradient::BestTracker::new();
