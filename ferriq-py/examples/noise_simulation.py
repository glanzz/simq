...
noise models in Ferriq. We'll compare ideal and noisy simulations.
"""
...
def main():
    ...
    ideal_config = ferriq.SimulatorConfig(shots=n_shots)
    ideal_sim = ferriq.Simulator(ideal_config)
    ...
    depol_channel = ferriq.DepolarizingChannel(0.05)  # 5% error rate
    noise_model.add_gate_error("cx", depol_channel)
    
    print("Noise model: 5% depolarizing error on CNOT gates")
    
    noisy_config = ferriq.SimulatorConfig(noise_model=noise_model, shots=n_shots)
    noisy_sim = ferriq.Simulator(noisy_config)
    ...
    noise_model2 = ferriq.HardwareNoiseModel()
    amp_damp = ferriq.AmplitudeDamping(0.1)  # 10% amplitude damping
    noise_model2.add_gate_error("cx", amp_damp)
    
    print("Noise model: 10% amplitude damping on CNOT gates")
    
    noisy_config2 = ferriq.SimulatorConfig(noise_model=noise_model2, shots=n_shots)
    noisy_sim2 = ferriq.Simulator(noisy_config2)
