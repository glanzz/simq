...
use pyo3::prelude::*;
use rand::Rng;
use ferriq_core::noise::types::NoiseChannel;
...
impl PySimulator {
        ...
        for gate_op in circuit.inner.operations() {
            ...
            if let Some(matrix_flat) = gate.matrix() {
                ...
                if n == 1 {
                    ...
                    state
                        .apply_single_qubit_gate(&matrix, qubits[0].into())
                        .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
                } else if n == 2 {
                    let matrix = [
                        [
                    ...
                    state
                        .apply_two_qubit_gate(&matrix, qubits[0].into(), qubits[1].into())
                        .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
                } else {
                    return Err(pyo3::exceptions::PyNotImplementedError::new_err(format!(
                        "{}-qubit gates not yet supported",
                        n
                    )));
                }
            }
    ...
    fn apply_noise(
        ...
        noise_model: &PyHardwareNoiseModel,
        qubits: &[ferriq_core::QubitId],
    ) -> PyResult<()> {
        let n = qubits.len();
        let gate_noise = if n == 1 {
            noise_model
                .inner
                .single_qubit_gate_noise(qubits[0].into())
                .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?
        } else if n == 2 {
            noise_model
                .inner
                .two_qubit_gate_noise(qubits[0].into(), qubits[1].into())
                .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?
        } else {
            return Ok(());
        };
    ...
    fn apply_kraus_op(
        ...
        state: &mut AdaptiveState,
        qubit: usize,
        op: &ferriq_core::noise::types::KrausOperator,
    ) -> PyResult<()> {
        let data = &op.matrix;
        let matrix = [[data[0], data[1]], [data[2], data[3]]];

        state
            .apply_single_qubit_gate(&matrix, qubit)
            .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
