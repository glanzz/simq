...
This module provides a comprehensive library of quantum gates for the Ferriq framework.
...
Using gate classes::
    ...
    from ferriq.gates import HGate, CNOTGate
    
    h = HGate()
    cnot = CXGate(control=0, target=1)
"""

# The gate classes live on the `gates` submodule of the `_ferriq` extension
# module. `_ferriq` is a binary module, not a package, so the submodule must
# be pulled in as an attribute rather than via `from ._ferriq.gates import`.
from ._ferriq import gates as _rust_gates
