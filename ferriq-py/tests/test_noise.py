...
def test_noisy_simulation_depolarizing():
    ...
    config = ferriq.SimulatorConfig(shots=1000, noise_model=model)
    sim = ferriq.Simulator(config)
...
def test_readout_error():
    ...
    config = ferriq.SimulatorConfig(shots=100, noise_model=model)
    sim = ferriq.Simulator(config)
