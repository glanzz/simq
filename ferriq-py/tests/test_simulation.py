...
def test_simulator_config():
    config = ferriq.SimulatorConfig(shots=2048)
    assert config.shots == 2048
    
    # Test default values
    default_config = ferriq.SimulatorConfig()
    assert default_config.shots == 1024

def test_simulator_creation():
    sim = ferriq.Simulator()
    assert sim is not None
    
    config = ferriq.SimulatorConfig(shots=100)
    sim_with_config = ferriq.Simulator(config)
...
def test_single_qubit_gate():
    builder = ferriq.CircuitBuilder(1)
    builder.x(0)  # |1>
    circuit = builder.build()
