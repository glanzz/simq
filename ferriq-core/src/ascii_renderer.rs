...
//!
//! ```ignore
//! use ferriq_core::{Circuit, AsciiConfig, render_ascii};
//!
...
mod tests {
    ...
    fn test_get_terminal_size_tput_success() {
        ...
        let dir = std::env::temp_dir().join(format!("ferriq_fake_tput_ok_{}", std::process::id()));
    ...
    fn test_get_terminal_size_tput_failure() {
        ...
        let dir = std::env::temp_dir().join(format!("ferriq_fake_tput_fail_{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_get_terminal_size_stty_success() {
        ...
        let dir = std::env::temp_dir().join(format!("ferriq_fake_stty_ok_{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_get_terminal_size_stty_too_few_parts() {
        ...
        let dir = std::env::temp_dir().join(format!("ferriq_fake_stty_short_{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_get_terminal_size_stty_failure() {
        ...
        let dir = std::env::temp_dir().join(format!("ferriq_fake_stty_fail_{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
