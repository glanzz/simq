...
//!
//! **Note**: This debugger requires `ferriq-sim` and `ferriq-state` crates.
//!
//! # Example
//!
//! ```ignore
//! use ferriq_core::{Circuit, StatefulDebugger};
//!
...
impl<'a> StatefulDebugger<'a> {
    ...
    /// the debugger with ferriq-sim integration.
    ...
    /// Print state summary (placeholder without ferriq-sim)
    pub fn print_state_summary(&self) {
        println!("State execution requires ferriq-sim integration.");
        println!("Circuit position: step {}/{}", self.current_step, self.total_gates());
    }

    /// Create a state snapshot from raw amplitudes
    ///
    /// This is a utility method for use with ferriq-state.
