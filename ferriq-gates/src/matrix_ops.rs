...
//!
//! ```rust
//! use ferriq_gates::matrix_ops::{embed_gate_matrix, matrix_multiply, is_unitary};
//! use ferriq_gates::matrices::{PAULI_X, IDENTITY};
//!
...
//! use ferriq_gates::matrix_ops::circuit_matrix;
//! use ferriq_gates::standard::{Hadamard, PauliX};
//! use std::sync::Arc;
//!
//! let mut circuit = Circuit::new(2);
//! circuit.add_gate(Arc::new(Hadamard), &[ferriq_core::QubitId::new(0)]).unwrap();
//! circuit.add_gate(Arc::new(PauliX), &[ferriq_core::QubitId::new(1)]).unwrap();
//!
...
/// ```
pub fn circuit_matrix(circuit: &ferriq_core::Circuit) -> std::result::Result<Vec<Complex64>, String> {
...
pub fn circuit_matrix_with_provider<F>(
    circuit: &ferriq_core::Circuit,
    matrix_provider: F,
) -> std::result::Result<Vec<Complex64>, String>
where
    F: Fn(&dyn ferriq_core::gate::Gate) -> Option<Vec<Complex64>>,
{
    let num_qubits = circuit.num_qubits();
    let system_size = 1 << num_qubits;
...
mod tests {
    ...
    fn test_circuit_matrix() {
        ...
        let mut circuit = Circuit::new(2);
        circuit
            .add_gate(Arc::new(Hadamard), &[ferriq_core::QubitId::new(0)])
            .unwrap();
        circuit
            .add_gate(Arc::new(PauliX), &[ferriq_core::QubitId::new(1)])
            .unwrap();
