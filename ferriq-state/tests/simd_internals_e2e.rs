...
fn test_simd_single_qubit_hadamard_matches_scalar() {
    ...
    for num_qubits in 1..=6 {
        for qubit in 0..num_qubits {
            ...
            ferriq_state::simd::single_qubit::apply_gate_scalar(
                &mut state_scalar,
                &h,
                qubit,
                num_qubits,
    ...
    for num_qubits in 2..=5 {
        for q1 in 0..num_qubits {
            for q2 in 0..num_qubits {
                ...
                ferriq_state::simd::two_qubit::apply_gate_scalar(
                    &mut state_scalar,
                    &cnot,
                    q1,
                    q2,
...
fn test_cnot_scalar_vs_striped_consistency() {
    ...
    for ctrl in 0..num_qubits {
        for tgt in 0..num_qubits {
            ...
            ferriq_state::simd::controlled_gates::apply_cnot_scalar(
                &mut state_scalar,
                ctrl,
                tgt,
                num_qubits,
            );
            ferriq_state::simd::controlled_gates::apply_cnot_striped(
                &mut state_striped,
                ctrl,
                tgt,
                num_qubits,
...
fn test_cz_scalar_vs_striped_consistency() {
    ...
    for q1 in 0..num_qubits {
        for q2 in 0..num_qubits {
            ...
            ferriq_state::simd::controlled_gates::apply_cz_scalar(
                &mut state_scalar,
                q1,
                q2,
                num_qubits,
            );
            ferriq_state::simd::controlled_gates::apply_cz_striped(
                &mut state_striped,
                q1,
                q2,
                num_qubits,
...
fn test_controlled_u_scalar_vs_striped() {
    ...
    for ctrl in 0..num_qubits {
        for tgt in 0..num_qubits {
            ...
            ferriq_state::simd::controlled_gates::apply_controlled_u_scalar(
                &mut state_scalar,
                ctrl,
                tgt,
                &u_matrix,
            ...
            ferriq_state::simd::controlled_gates::apply_controlled_u_striped(
                &mut state_striped,
                ctrl,
                tgt,
                &u_matrix,
...
fn test_diagonal_scalar_vs_optimized() {
    ...
    for qubit in 0..num_qubits {
        ...
        ferriq_state::simd::diagonal::apply_diagonal_gate_scalar(
            &mut state_scalar,
            t_diag,
            qubit,
            num_qubits,
        );
        ferriq_state::simd::diagonal::apply_diagonal_gate_optimized(
            &mut state_opt,
            t_diag,
            qubit,
            num_qubits,
...
fn test_compute_probabilities_scalar_vs_dispatch() {
    ...
    ferriq_state::simd::kernels::compute_probabilities_scalar(&amplitudes, &mut probs_scalar);
    ferriq_state::simd::kernels::compute_probabilities(&amplitudes, &mut probs_dispatch);
...
fn test_compute_probabilities_sum_to_one_for_normalized_state() {
    ...
    ferriq_state::simd::kernels::compute_probabilities(&state, &mut probs);
...
fn test_apply_single_qubit_gate_hadamard_2qubit() {
    ...
    // In the ferriq convention, qubit 0 is the LSB of the index.
...
fn test_apply_cnot_10_becomes_11() {
    ...
    // In ferriq convention: index bit k = (index >> k) & 1
