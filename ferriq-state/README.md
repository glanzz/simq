...
High-performance quantum state representations with SIMD-optimized operations for the Ferriq quantum computing SDK.

## Overview

The `ferriq-state` crate provides efficient quantum state vector implementations with SIMD (Single Instruction, Multiple Data) optimized matrix-vector multiplication. This dramatically accelerates quantum circuit simulation by leveraging modern CPU vector instructions (SSE2, AVX, AVX-512).
...
```rust
use ferriq_state::simd::{norm_simd, normalize_simd};
...
```bash
cargo bench --package ferriq-state
```
...
```bash
cargo test --package ferriq-state
```
...
MIT OR Apache-2.0 (same as Ferriq project)
