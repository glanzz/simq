//! Edge case and error path tests for ferriq-backend

use ferriq_backend::*;
use ferriq_core::{Circuit, QubitId};
use ferriq_gates::{CNot, Hadamard, PauliX, PauliZ, TGate};
...
mod error_edge_cases {
    ...
    fn from_quantum_error() {
        let core_err = ferriq_core::QuantumError::InvalidQubit(99, 2);
...
mod gate_decomposer_edge_cases {
    ...
    fn decompose_native_gate_returns_self() {
        ...
        let op = ferriq_core::GateOp::new(Arc::new(PauliX), &[q0]).unwrap();
    ...
    fn decompose_h_to_ibm() {
        let decomposer = GateDecomposer::ibm_native();
        let q0 = QubitId::new(0);
        let op = ferriq_core::GateOp::new(Arc::new(Hadamard), &[q0]).unwrap();
        let result = decomposer.decompose_gate(&op).unwrap();
    ...
    fn decompose_unknown_gate_errors() {
        ...
        let op = ferriq_core::GateOp::new(
            Arc::new(ferriq_gates::Fredkin),
