//! Edge case and error path tests for simq-gates
...
mod matrix_ops_edge_cases {
    ...
    fn determinant_2x2_identity() {
        let id = simq_gates::matrices::IDENTITY;
    ...
    fn fidelity_orthogonal_unitaries() {
        let x = matrix_ops::matrix_to_vec(&simq_gates::matrices::PAULI_X);
        let z = matrix_ops::matrix_to_vec(&simq_gates::matrices::PAULI_Z);
    ...
    fn vec_to_matrix_2x2_roundtrip() {
        let original = simq_gates::matrices::HADAMARD;
    ...
    fn embed_single_qubit_in_3_qubit_system() {
        let x = &simq_gates::matrices::PAULI_X;
...
mod lookup_edge_cases {
    ...
    fn large_angle_fallback() {
        ...
        let direct = simq_gates::matrices::rotation_x(PI);
        for i in 0..2 {
            for j in 0..2 {
    ...
    fn interpolation_disabled_vs_enabled() {
        ...
        let direct = simq_gates::matrices::rotation_x(angle);
...
mod generated_cache_edge_cases {
    ...
    fn qaoa_out_of_range_fallback() {
        ...
        let direct = simq_gates::matrices::rotation_x(-1.0);
        for i in 0..2 {
            for j in 0..2 {
    ...
    fn qaoa_rz_out_of_range() {
        ...
        let direct = simq_gates::matrices::rotation_z(PI + 1.0);
        for i in 0..2 {
            for j in 0..2 {
                assert!((result[i][j] - direct[i][j]).norm() < 1e-10);
            }
    ...
    fn enhanced_cache_all_rotation_axes() {
        ...
        let direct_rx = simq_gates::matrices::rotation_x(PI / 4.0);
        let direct_ry = simq_gates::matrices::rotation_y(PI / 4.0);
        let direct_rz = simq_gates::matrices::rotation_z(PI / 4.0);
...
mod standard_gate_edge_cases {
    ...
    fn all_single_qubit_gates_are_unitary() {
        ...
        for gate in &gates {
            ...
            assert!(
                simq_gates::is_unitary(&matrix, 1e-10),
    ...
    fn hermitian_gates_are_self_adjoint() {
        ...
        for gate in &hermitian_gates {
            ...
            assert!(
                simq_gates::is_hermitian(&m, 1e-10),
